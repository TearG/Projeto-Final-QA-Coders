...
Então('devo obter a informacao {string}') do |verifica_sobre_nos|
...
Então('devo obter o elemento {string}') do |verifica_parceiros|
    home.validate_text_menu_parceiros(verifica_parceiros)
    sleep 2
end

Quando('clicar sobre o menu {string}') do |faleConosco| 
    home.click_link(faleConosco) 
     sleep 2
end
Então('devo obter a pagina {string}') do |verifica_faleConosco|
